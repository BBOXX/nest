...
def retry_valid_input(prompt, title = '', default = None, condition=lambda x: x is not None, transform=lambda x: x):
    ...
    Keyword Arguments:
        ...
        default {Any} -- Default value of user input. Be careful that default passes condition. (default: {None})
        condition {function: a -> bool} -- Condition returning boolean to signal correct input from user. (default: {lambdax:x is not None})
        transform {function: a -> b} -- Trasnformation function transforming user data before returning it. (default: {lambda x:x})
    
    Returns:
        b -- Returns result of transform function (which defaults to an identity function)
    ...
    while True:
        ...
        # If user_input passes the condition, don't try again and transform and save the output
...
def save_config(config, config_file = None):
    ...
    Keyword Arguments:
        config_file {str (path)} -- Path to config file. If not specified will ask the user. (default: {None})
...
def main():
    return save_config(make_config())
